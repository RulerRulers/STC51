 ...
 * File Description : Infra-red cotrol pad demo dialog
...
static void InitIrDemo(void)
{
	const struct gui_rect code txt_rect= {5, 40, LCD_WIDTH - 10, LCD_HEIGHT - 80};
	struct dlg_ir_demo_param *p = &g_all_dlg.ir_demo;
...
static void DumpIrInfo(uint8_t msg_type, uint8_t msg_code)
{
	const struct gui_rect code txt_rect= {5, 40, LCD_WIDTH - 10, LCD_HEIGHT - 80};
...
void gui_DlgIrDemoProc(uint8_t msg_type, uint8_t msg_code)
{
	if(msg_type == WM_KEYPRESSED || msg_type == WM_KEYRELEASED || msg_type == WM_KEYREPEATED)
	{
		DumpIrInfo(msg_type, msg_code);
	}
