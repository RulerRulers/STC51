...
static FATFS xdata *FatFs;	/* Pointer to the file system object (logical drive) */


/* Fill memory */
static
void mem_set (void* dst, int val, int cnt) large
{
	char *d = (char*)dst;
...
{
	BYTE xdata c, ni, si, i;
	BYTE *sfn;
	const char *p;
#if _USE_LCC
#ifdef _EXCVT
	static const BYTE code cvt[] = _EXCVT;
#endif
#endif
...
FRESULT pf_mount (
	FATFS xdata *fs		/* Pointer to new file system object */
)
{
	BYTE xdata fmt, buf[36];
	DWORD xdata bsect, fsize, tsect, mclst;
...
)
{
	FRESULT xdata res;
	DIR xdata dj;
	BYTE xdata sp[12], dir[32];
	FATFS xdata *fs = FatFs;
